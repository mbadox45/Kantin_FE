<script setup>
    ...
    import {getAllShift, postShift, putShift} from '@/controllers/masterShiftController'
    ...
    const post_data_input = async() => {
        try {
            if (form.value.nama_shift != null && form.value.jam_mulai != null && form.value.jam_selesai != null) {
                if (status_form.value == 'add') {
                    const response = await postShift(form.value)
                    ...
                    const response = await putShift(form.value.id, form.value)
                    if (response.status == true) {
                        messages.value = [
...
<template>
    <div class="flex flex-column gap-5">
        ...
        <div class="flex justify-content-between align-items-center">
            <strong class="text-3xl font-medium uppercase">Master Shift</strong>
            <small class="flex align-items-center">Master <strong class="mx-2">/</strong> <i class="mx-2 text-sm pi pi-bolt"></i>Shift</small>
        </div>
        ...
        <Dialog v-model:visible="visible_dialog" modal :header="header_dialog" :style="{ width: '600px' }">
            ...
            <div class="flex flex-column gap-3">
                <div class="flex flex-column justify-content-between w-full gap-1">
                    <label for="username" class="font-semibold w-6rem">Nama Shift</label>
                    <InputText id="username" v-model="form.nama_shift" class="w-full" autocomplete="off" />
                </div>
                <div class="flex gap-3 align-items-center">
                    <div class="flex flex-column justify-content-between w-full gap-1">
                        <label for="masuk" class="font-semibold w-6rem">Jam Masuk</label>
                        <InputText id="masuk" type="time" v-model="form.jam_mulai" class="w-full" autocomplete="off" />
                    </div>
                    <div class="flex flex-column justify-content-between w-full gap-1">
                        <label for="pulang" class="font-semibold w-6rem">Jam Pulang</label>
        ...
        <div class="flex justify-content-center align-items-center gap-3">
            <div class="w-full flex flex-column bg-white p-3 border-round shadow-2 gap-3">
                <div class="flex justify-content-between align-items-center">
                    <strong class="uppercase text-md font-medium w-full"><i class="pi pi-list mr-3"></i>List Master Shift</strong>
                ...
                <div class="w-full flex flex-column">
                    <DataTable v-model:filters="filters" :value="list_user_akses" paginator :rows="10"
                        :globalFilterFields="['nama_shift', 'jam_selesai', 'jam_mulai']" tableStyle="min-width: 50rem">
                        <Column header="Nama Shift" sortable sortField="nama_shift">
                            <template #body="{ data }">
                                <div class="flex align-items-center gap-2">
                                    <div class="flex flex-column gap-1">
                                        <strong class="uppercase">{{ data.nama_shift }}</strong>
                                    </div>
                                </div>
                            </template>
                        </Column>
                        <Column header="Jam Mulai" sortable sortField="jam_mulai">
                            <template #body="{ data }">
                                <div class="flex align-items-center gap-2">
                        ...
                        <Column header="Jam Selesai" sortable sortField="jam_selesai">
                            <template #body="{ data }">
                                <div class="flex align-items-center gap-2">
